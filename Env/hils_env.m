...
rosinit('http://10.42.0.120:11311');
...
target_pose_sub = rossubscriber('/mavros/setpoint_position/local', 'geometry_msgs/PoseStamped');

while(1)
    send(map_pub, map_msg);
    
    target_pose_msg = receive(target_pose_sub, 10);
    target_pose(1) = target_pose_msg.Pose.Position.X;
    target_pose(2) = target_pose_msg.Pose.Position.Y;
    target_pose(3) = target_pose_msg.Pose.Position.Z;
    
    %goal_gap = current_pose - target_pose;
    %goal_dist = sqrt(sum(goal_gap.^2));

    
    current_pose_msg.Pose.Position.X = target_pose(1);
    current_pose_msg.Pose.Position.Y = target_pose(2);
    current_pose_msg.Pose.Position.Z = target_pose(3);
    send(current_pose_pub, current_pose_msg);
    

    gas_msg.Values(1) = g(round(target_pose(2)/3),round(target_pose(1)/3),round(target_pose(3)/3));
    ...
    if current_pose_msg.Pose.Position.Z < 30
        for jj = 1:lidar_num
            ...
            for ii = 1:lidar_length
                ...
                x_r(jj) = round(target_pose(1)+cos(jjj)*(ii+0.1));
                y_r(jj) = round(target_pose(2)+sin(jjj)*(ii+0.1));
        ...
        lidar_msg.Ranges = [];        
        if isnan( map_nan( round(target_pose(2)), round(target_pose(1)) ) )
            lidar_msg.Ranges(1) = target_pose(3) - 30;
        end
    end
